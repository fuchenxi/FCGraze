...
@property (nonatomic, copy) FCCenterFailureProcessBlock failureProcessHandler;
...
- (void)setFailureProcessBlock:(FCCenterFailureProcessBlock)block {
    
    self.failureProcessHandler = block;
}
...
+ (void)setFailureProcessBlock:(FCCenterFailureProcessBlock)block {
    ...
    [[self defaultCenter] setFailureProcessBlock:block];
}
...
- (void)fc_execureFailureBlockWithError:(id)error forRequest:(FCRequest *)request {
    ...
    FC_SAFE_BLOCK(self.failureProcessHandler, request, error);
